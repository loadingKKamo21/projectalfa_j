...
(
    ...
    username               VARCHAR(255) UNIQUE NOT NULL COMMENT '아이디',
    password               VARCHAR(255)        NOT NULL COMMENT '비밀번호',
    auth                   TINYINT(1) NOT NULL DEFAULT 0 COMMENT '인증 여부',
    authenticated_time     DATETIME COMMENT '계정 인증 시각',
    email_auth_token       VARCHAR(255) COMMENT '이메일 인증 토큰',
    email_auth_expire_time DATETIME COMMENT '이메일 인증 만료 시간',
    o_auth_provider        VARCHAR(255) COMMENT 'OAuth 2.0 Provider',
    o_auth_provider_id     VARCHAR(255) COMMENT 'OAuth 2.0 Provider Id',
    nickname               VARCHAR(255) UNIQUE NOT NULL COMMENT '닉네임',
    signature              VARCHAR(255) COMMENT '서명',
    role                   VARCHAR(255)        NOT NULL COMMENT '계정 유형',
...
(
    ...
    store_filename     VARCHAR(255) NOT NULL COMMENT '저장 파일명',
    store_file_path    VARCHAR(255) NOT NULL COMMENT '저장 경로',
